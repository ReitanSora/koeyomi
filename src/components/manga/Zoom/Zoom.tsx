...
import Animated, { AnimatableValue, AnimationCallback, runOnJS, SharedValue, useAnimatedStyle, useSharedValue, withDecay, withTiming } from 'react-native-reanimated';
...
export function useZoomGesture(props: UseZoomGestureProps = {}): {
    ...
    const { animationFunction = withTiming, animationConfig, doubleTapConfig } = props;
    ...
    const handlePanOutsideTimeoutId: React.RefObject<ReturnType<typeof setTimeout> | undefined> = useRef(undefined);
    ...
    const zoomIn = useCallback((): void => {
        ...
        const clampedScale = clampScale(newScale, doubleTapConfig?.minZoomScale ?? MIN_SCALE, doubleTapConfig?.maxZoomScale ?? MAX_SCALE);
    ...
    }, [baseScale, pinchScale, lastOffsetX, lastOffsetY, translateX, translateY, isZoomedIn, lastScale, getContentContainerSize, withAnimation, doubleTapConfig]);
    ...
    }, [baseScale, pinchScale, lastOffsetX, lastOffsetY, translateX, translateY, lastScale, isZoomedIn, withAnimation]);

    const handlePanOutside = useCallback((): void => {
        if (handlePanOutsideTimeoutId.current !== undefined) clearTimeout(handlePanOutsideTimeoutId.current);
    ...
    }, [lastOffsetX, lastOffsetY, lastScale, translateX, translateY, containerDimensions, getContentContainerSize, withAnimation]);
    ...
    const zoomGesture = useMemo(() => {
        ...
        const panGesture = Gesture.Pan()
            ...
            .onUpdate((event: GestureUpdateEvent<PanGestureHandlerEventPayload>): void => {
                ...
                translateX.value = lastOffsetX.value + translationX / lastScale.value / pinchScale.value;
                translateY.value = lastOffsetY.value + translationY / lastScale.value / pinchScale.value;
            })
        ...
        return Gesture.Simultaneous(tapGesture, panGesture, pinchGesture, singleTap);
    }, [
        handlePanOutside,
        lastOffsetX,
        lastOffsetY,
    ...
    const contentContainerAnimatedStyle = useAnimatedStyle(() => ({
        transform: [{ scale: baseScale.value * pinchScale.value }, { translateX: translateX.value }, { translateY: translateY.value }],
    }));
...
export interface ZoomProps {
    ...
    animationFunction?<T extends AnimatableValue>(toValue: T, userConfig?: AnimationConfigProps, callback?: AnimationCallback): T;
}
